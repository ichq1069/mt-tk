...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Enter" && inputValue.trim()) {
      e.preventDefault();
      // 如果有完全匹配的项，则选择
      const matched = options.find(opt => opt.label.toLowerCase() === inputValue.trim().toLowerCase());
      if (matched) {
        if (!selectedOptions.includes(matched.value)) {
          handleSelect(matched.value);
        }
      } else {
        // 如果没有匹配项且不为空，则作为一个新值（直接把文本作为 value 和 label）
        ...
        <div onClick={toggleDropdown} className="w-full">
          <div className="mb-2 flex h-11 rounded-lg border border-gray-300 py-1.5 pl-3 pr-3 shadow-theme-xs outline-hidden transition focus:border-brand-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-brand-300">
            <div className="flex flex-wrap flex-auto gap-2 items-center">
              ...
              <input
                placeholder={selectedOptions.length > 0 ? "" : "选择选项..."}
                className="flex-1 h-8 min-w-[60px] p-1 text-sm bg-transparent border-0 outline-hidden appearance-none placeholder:text-gray-400 focus:border-0 focus:outline-hidden focus:ring-0 dark:placeholder:text-gray-500"
                value={inputValue}
                disabled={disabled}
                onChange={(e) => {
