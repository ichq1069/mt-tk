...
- `getRandomMedia(params)`: 随机获取一批媒体作品（图片/视频通用）。
- `getRandomImages(params)`: 随机获取一批美图/精选图片（语义别名，等同于 `getRandomMedia({ type: 'image', ...params })`）。
  - `params.count`: 返回数量（默认 10）
  - `params.categoryId`: 按分类筛选
  - `params.tag`: 按标签筛选
  - `params.type`: 媒体类型（`getRandomMedia` 专用，可选 `'image'|'video'`，默认 `'image'`）
