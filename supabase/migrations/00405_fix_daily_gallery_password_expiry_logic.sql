-- 1. 修正已有数据，将所有历史发布记录的密码有效期延长到未来
UPDATE daily_gallery_posts 
SET password_expires_at = NOW() + INTERVAL '100 years'
WHERE is_published = TRUE;

-- 2. 改进验证函数，针对每日密码放宽过期限制
...
BEGIN
  ...
  IF p_password IS NOT NULL AND v_post.password = p_password THEN
    -- 对于每日密码，只要密码匹配，即使 expires_at 数据库存的是过去的，也允许访问（因为每日密码属于该日期的基础属性）
    -- 除非专门针对当天的发布做了某种短期时效性要求
