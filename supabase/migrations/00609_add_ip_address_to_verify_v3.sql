...
BEGIN
    ...
    -- 标识符逻辑优化：
    -- 对于多用户密码，使用 IP 地址作为标识，锁定同一网络下的访问
    -- 对于其他密码，优先使用 browser_id (浏览器指纹) 或 openid
    ...
    ELSE
        ...
        IF v_special_pwd.id IS NOT NULL THEN
            ...
            -- 3.1.3 专属校验：针对【微信公众号申请】(source='wechat') 的定期密码（单用户/多用户）进行 OpenID 强匹配
            ...
            ELSIF v_special_pwd.password_type IN ('periodic_single_user', 'periodic_multi_user') THEN
                IF v_special_pwd.password_type = 'periodic_single_user' THEN
                    ...
                    LIMIT 1;
                ELSE
                    -- 多用户定期密码：允许多人使用，但每个人仅限在自己首个激活的设备上使用
                    -- 这里的逻辑改为：如果提供了 IP，则使用 IP 作为锁定标识，否则回退到浏览器指纹
                    -- 这样可以防止同一网络下的多个浏览器/设备共享多用户密码
                    v_identifier := COALESCE(NULLIF(TRIM(p_ip_address), ''), v_identifier);
