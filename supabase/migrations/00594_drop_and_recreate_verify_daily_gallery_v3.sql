...
BEGIN
    ...
    ELSE
        ...
        IF v_special_pwd.id IS NOT NULL THEN
            -- 3.1.1 检查是否为专属密码 (wechat/miniprogram)
            IF v_special_pwd.source IN ('wechat', 'miniprogram') THEN
                -- 如果没有 openid (手动输入)
                IF p_openid IS NULL OR p_openid = '' THEN
                    RETURN jsonb_build_object(
                        'success', false, 
                        'message', '该密码请从微信公众号生成的链接跳转打开', 
            ...
            -- 3.1.2 校验浏览器锁定 (针对定期密码) - 此时说明是通用密码或者已通过专属校验的密码
