...
begin
    ...
    -- 设备终端分布 (简单判断: 如果 OS 包含 Android/iOS/HarmonyOS 则是移动端)
    select json_agg(t) into device_dist from (
        select 
            case 
                when os ilike '%Android%' or os ilike '%iPhone%' or os ilike '%iPad%' or os ilike '%iOS%' or os ilike '%Harmony%' then 'Mobile'
                else 'PC'
            end as name,
            count(*) as value
        from user_visit_stats
