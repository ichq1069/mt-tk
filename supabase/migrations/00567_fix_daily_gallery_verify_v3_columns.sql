-- 修复 verify_daily_gallery_v3 中的列名错误 (usages -> used_count, last_used_at -> used_at)
-- 并优化 max_usages 的逻辑，确保 0 被视为 1 或处理异常
...
BEGIN
    ...
    ELSE
        -- 3.1 检查特权/一次性密码
        -- 注意：这里使用了正确的列名 used_count 和 max_usages
        -- 处理 max_usages 为 0 的情况，将其视为 1
        ...
        WHERE password = p_password
          ...
          AND (
            is_one_time = false 
            OR (is_one_time = true AND used_count < COALESCE(NULLIF(max_usages, 0), 1))
          )
        LIMIT 1;
        ...
        ELSIF v_enable_password = false THEN
            -- 如果全局禁用了密码，但输入了不匹配的密码，依然允许通过（除非以后有更严格要求）
            v_expires_at := now() + interval '24 hours';
        ELSE
