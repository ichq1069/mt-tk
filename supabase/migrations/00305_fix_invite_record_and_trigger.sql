-- Update handle_new_user function to include referrer_id and group_id
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    group_id = COALESCE(EXCLUDED.group_id, profiles.group_id);
...
SET 
  referrer_id = (u.raw_user_meta_data->>'referrer_id')::uuid,
  group_id = COALESCE((u.raw_user_meta_data->>'group_id')::uuid, p.group_id)
FROM auth.users u
WHERE p.id = u.id 
  AND (u.raw_user_meta_data->>'referrer_id' IS NOT NULL OR u.raw_user_meta_data->>'group_id' IS NOT NULL);
