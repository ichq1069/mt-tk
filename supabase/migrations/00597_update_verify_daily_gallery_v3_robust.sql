...
BEGIN
    ...
    ELSE
        ...
        -- 优化：不仅匹配当前日期，如果匹配到其他日期的有效密码，给出明确提示
        ...
        ORDER BY (target_date = v_target_date) DESC, target_date DESC NULLS LAST
        LIMIT 1;
        ...
        IF v_special_pwd.id IS NOT NULL THEN
            ...
            IF v_special_pwd.target_date IS NOT NULL AND v_special_pwd.target_date <> v_target_date THEN
                RETURN jsonb_build_object('success', false, 'message', '该密码不适用于' || p_post_date || '的图集', 'errorCode', 'DATE_MISMATCH');
            END IF;

            -- 3.1.1 专属密码校验 (针对 wechat 来源)
            IF v_special_pwd.source = 'wechat' THEN
                -- 如果是手动输入 (未提供 openid)，提示从公众号跳转
                IF p_openid IS NULL OR p_openid = '' THEN
                    RETURN jsonb_build_object(
                        'success', false, 
                        'message', '该密码为专属密码，请从微信公众号生成的链接跳转打开', 
                        'errorCode', 'WECHAT_EXCLUSIVE_PASSWORD'
                    );
                ELSIF p_openid <> v_special_pwd.creator_id THEN
                    -- 如果 openid 提供了但不匹配
                    RETURN jsonb_build_object('success', false, 'message', '密码错误，非您的专属密码', 'errorCode', 'INVALID_PASSWORD');
                END IF;
            END IF;

            -- 3.1.2 校验浏览器锁定 (针对定期密码)
            IF v_special_pwd.password_type = 'periodic' AND v_special_pwd.browser_id IS NOT NULL AND v_special_pwd.browser_id <> '' THEN
                IF v_identifier IS NOT NULL AND v_identifier <> '' AND v_special_pwd.browser_id <> v_identifier THEN
                    RETURN jsonb_build_object('success', false, 'message', '该密码已在其他浏览器中使用', 'errorCode', 'BROWSER_LOCKED');
                END IF;
            END IF;
        ...
        ELSE
            ...
            ELSE
                ...
                IF v_user_pwd.id IS NOT NULL THEN
                    ...
                    END IF;
                ELSE
                    RETURN jsonb_build_object('success', false, 'message', '密码错误，请核对后重试', 'errorCode', 'INVALID_PASSWORD');
                END IF;
            END IF;
        END IF;
    END IF;
