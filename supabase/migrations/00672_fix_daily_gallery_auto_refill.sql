...
BEGIN
    ...
    -- [NEW] Clear pending status for items that no longer meet requirements 
    -- (e.g. recently used in WeChat, hidden, or category/tag excluded)
    ...
    WHERE daily_gallery_status = 'pending'
      AND (
          ...
          OR (wechat_draft_status IN ('used', 'adopted') AND wechat_last_used_at >= v_fifteen_days_ago)
