-- 删除所有现有版本的函数，彻底清理冲突
DROP FUNCTION IF EXISTS public.get_random_media_items(integer, uuid);
DROP FUNCTION IF EXISTS public.get_random_media_items(uuid, integer);
...
BEGIN
    ...
    SELECT 
        ...
        m.created_at, 
        m.status::text, -- 显式转换枚举为 TEXT，修复 structure mismatch 错误
        m.deleted_at,
        p.username, 
        p.avatar_url,
        COALESCE(
