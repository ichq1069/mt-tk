...
DECLARE
    ...
    v_target_date date;
    v_identifier text;
    v_lock_identifier text;
BEGIN
    -- 锁定标识优先使用 browser_id
    v_lock_identifier := COALESCE(p_browser_id, p_openid);
    -- 行为记录标识优先使用 openid
    ...
    ELSE
        -- 3.1 优先检查随机密码库 (daily_gallery_special_passwords)
        -- 改进：增加排序优先级，优先匹配当前用户的记录，避免因 6 位数字密码碰撞导致的锁定冲突
        ...
        ORDER BY 
          (CASE 
            WHEN creator_id = p_openid THEN 1 
            WHEN browser_id = v_lock_identifier THEN 2 
            ELSE 3 
          END),
          target_date DESC NULLS LAST
        LIMIT 1;
        ...
        IF v_special_pwd.id IS NOT NULL THEN
            ...
            IF v_special_pwd.password_type = 'periodic' AND v_special_pwd.browser_id IS NOT NULL AND v_special_pwd.browser_id <> '' THEN
                IF v_lock_identifier IS NOT NULL AND v_lock_identifier <> '' AND v_special_pwd.browser_id <> v_lock_identifier THEN
            ...
            SET used_count = COALESCE(used_count, 0) + 1, 
                ...
                browser_id = CASE 
                    ...
                    THEN v_lock_identifier ELSE browser_id END
        ...
        -- 3.4 检查旧的用户专属密码 (daily_gallery_user_passwords) - 兼容处理
        ELSE
            SELECT * INTO v_user_pwd
            FROM public.daily_gallery_user_passwords
            WHERE (openid = p_openid OR locked_browser_id = p_browser_id)
