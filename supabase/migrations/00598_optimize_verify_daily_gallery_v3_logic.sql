        ...
        IF v_special_pwd.id IS NOT NULL THEN
            ...
            IF v_special_pwd.source = 'wechat' THEN
                ...
                IF p_openid IS NULL OR p_openid = '' THEN
                    -- 如果密码属于当前用户 (browser_id 匹配)，但没有 openid，也允许（支持缓存场景）
                    -- 但如果完全没有凭证，则提示
                    IF v_special_pwd.browser_id IS NOT NULL AND v_special_pwd.browser_id <> '' AND v_special_pwd.browser_id <> v_identifier THEN
                         RETURN jsonb_build_object(
                            'success', false, 
                            'message', '该内容为专属访问，请通过公众号发送的专属链接打开', 
