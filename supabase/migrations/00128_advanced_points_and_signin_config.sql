-- Add columns to signin_configs
ALTER TABLE signin_configs ADD COLUMN IF NOT EXISTS is_bonus BOOLEAN DEFAULT FALSE;
ALTER TABLE signin_configs ADD COLUMN IF NOT EXISTS bonus_note TEXT;

-- Create system_configs entry for points_logic if not exists
INSERT INTO system_configs (key, value)
VALUES ('points_logic', '{
  "image_publish": { "points": 10, "exp": 5, "limit": 3, "period": 1 },
  "video_publish": { "points": 20, "exp": 10, "limit": 2, "period": 1 },
  "favorite": { "points": 2, "exp": 1, "limit": 10, "period": 1 },
  "comment": { "points": 5, "exp": 2, "limit": 5, "period": 1 },
  "report": { "points": 10, "exp": 5, "limit": 5, "period": 1 },
  "daily_login": { "points": 5, "exp": 2, "limit": 1, "period": 1 }
}')
ON CONFLICT (key) DO NOTHING;

-- Create RPC for awarding rewards with limit check
...
BEGIN
    -- 1. 获取配置
    SELECT value INTO v_logic FROM public.system_configs WHERE key = 'points_logic';
