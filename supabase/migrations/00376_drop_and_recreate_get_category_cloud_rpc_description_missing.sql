DROP FUNCTION IF EXISTS public.get_category_cloud();

CREATE OR REPLACE FUNCTION public.get_category_cloud()
 RETURNS TABLE(id uuid, name text, icon text, sort_order integer, is_visible boolean, min_role text, count bigint)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
