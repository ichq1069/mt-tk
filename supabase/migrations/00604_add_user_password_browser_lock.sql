...
-- 修改 verify_daily_gallery_v3，增加对 daily_gallery_user_passwords 的浏览器锁定校验
...
BEGIN
    ...
    -- 确定当前访问者的唯一标识符 (优先使用浏览器指纹，公众号环境回退到 openid)
    ...
    ELSE
        ...
        IF v_special_pwd.id IS NOT NULL THEN
            ...
            -- 3.1.4 核心改进：浏览器指纹级别锁定
            IF v_special_pwd.browser_id IS NOT NULL AND v_special_pwd.browser_id <> '' THEN
                IF v_identifier IS NULL OR v_identifier = '' OR v_identifier <> v_special_pwd.browser_id THEN
                    RETURN jsonb_build_object(
                        'success', false, 
        ...
        ELSE
            ...
            ELSE
                ...
                IF v_user_pwd.id IS NOT NULL THEN
                    ...
                    -- 新增：对 daily_gallery_user_passwords 进行浏览器指纹级别锁定
                    IF v_user_pwd.locked_browser_id IS NOT NULL AND v_user_pwd.locked_browser_id <> '' THEN
                        IF v_identifier IS NULL OR v_identifier = '' OR v_identifier <> v_user_pwd.locked_browser_id THEN
                            RETURN jsonb_build_object(
                                'success', false, 
                                'message', '该密码已在其他设备或浏览器激活，专属密码仅支持首个打开的设备。', 
                                'errorCode', 'BROWSER_LOCKED'
