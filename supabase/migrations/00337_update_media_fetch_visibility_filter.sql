...
BEGIN
    ...
    v_excluded_category_ids := ARRAY(
        SELECT cc_id FROM (
            SELECT unnest(v_excluded_category_ids) as cc_id
            UNION
            SELECT id FROM public.content_categories WHERE is_visible = false
        ) s
    );
    
    v_excluded_tag_ids := ARRAY(
        SELECT t_id FROM (
            SELECT unnest(v_excluded_tag_ids) as t_id
            UNION
            SELECT id FROM public.tags WHERE is_visible = false
        ) s
    );
