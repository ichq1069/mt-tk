-- Ensure the function is defined correctly and refresh the schema cache
DROP FUNCTION IF EXISTS public.get_optimized_media_items_v3(uuid, text, text, uuid[], text, integer, integer);
...
BEGIN
    ...
    IF p_user_id IS NOT NULL THEN
        SELECT 
            COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_prof.custom_fields->'preferences'->'disliked_categories')::uuid), ARRAY[]::uuid[]),
            COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_prof.custom_fields->'preferences'->'disliked_tags')::uuid), ARRAY[]::uuid[])
        INTO v_excluded_category_ids, v_excluded_tag_ids
        FROM public.profiles AS p_prof
        WHERE p_prof.id = p_user_id;
    END IF;
    ...
    IF p_sort_by = 'latest' THEN
        ...
        WHERE m.status::text = 'approved' AND m.deleted_at IS NULL AND m.is_hidden = false
