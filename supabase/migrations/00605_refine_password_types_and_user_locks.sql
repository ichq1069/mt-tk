-- 重构每日图集密码类型，支持 per-user 浏览器锁定
-- 解决：a用户使用后b用户无法使用的问题

-- 1. 创建 per-user 浏览器锁定记录表
CREATE TABLE public.daily_gallery_password_user_locks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  password_id UUID NOT NULL REFERENCES public.daily_gallery_special_passwords(id) ON DELETE CASCADE,
  user_identifier TEXT NOT NULL,
  browser_id TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 联合唯一索引：同一个密码同一个用户只能有一条锁定记录
CREATE UNIQUE INDEX idx_password_user_locks_unique ON public.daily_gallery_password_user_locks(password_id, user_identifier);
CREATE INDEX idx_password_user_locks_password_id ON public.daily_gallery_password_user_locks(password_id);
CREATE INDEX idx_password_user_locks_user ON public.daily_gallery_password_user_locks(user_identifier);

-- RLS
ALTER TABLE public.daily_gallery_password_user_locks ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow admins full access on password user locks" ON public.daily_gallery_password_user_locks
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role::text = 'admin'));

-- 2. 更新现有 password_type 值，兼容新旧类型
-- periodic_single_user: 单用户定期密码（per-user 浏览器锁定）
-- periodic_multi_user: 多用户定期密码（per-user 浏览器锁定，多个用户可用）
-- long_term: 长期使用（原 permanent）

-- 先将旧的 periodic 映射为 periodic_multi_user（因为旧的 periodic 就是公用密码）
UPDATE public.daily_gallery_special_passwords 
SET password_type = 'periodic_multi_user' 
WHERE password_type = 'periodic';

-- 将 permanent 映射为 long_term
UPDATE public.daily_gallery_special_passwords 
SET password_type = 'long_term' 
WHERE password_type = 'permanent';

-- 3. 重写 verify_daily_gallery_v3 函数
DROP FUNCTION IF EXISTS public.verify_daily_gallery_v3(text, text, text, text);
...
BEGIN
    ...
    ELSE
        ...
        IF v_special_pwd.id IS NOT NULL THEN
            ...
            -- 3.1.3 单用户密码来源校验 (periodic_single_user 或 source='wechat')
            IF v_special_pwd.password_type = 'periodic_single_user' OR v_special_pwd.source = 'wechat' THEN
            ...
            -- 类型 2 & 3: periodic_single_user / periodic_multi_user（定期密码，per-user 浏览器锁定）
            ELSIF v_special_pwd.password_type IN ('periodic_single_user', 'periodic_multi_user') THEN
                ...
                IF v_user_lock.id IS NOT NULL THEN
                    -- 该用户已有锁定，检查是否匹配当前浏览器
                    IF v_user_lock.browser_id <> v_identifier THEN
                        RETURN jsonb_build_object(
                            'success', false, 
        ...
        ELSE
            ...
            ELSE
                ...
                IF v_user_pwd.id IS NOT NULL THEN
                    ...
                    -- daily_gallery_user_passwords 浏览器指纹级别锁定（per-user 已有独立记录）
