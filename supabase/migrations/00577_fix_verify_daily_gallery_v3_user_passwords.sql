            ...
            IF v_user_pwd.id IS NOT NULL THEN
                ...
                IF v_identifier IS NOT NULL AND v_identifier <> '' THEN
                    IF v_user_pwd.locked_browser_id IS NULL OR v_user_pwd.locked_browser_id = '' THEN
                        -- 首次使用，锁定浏览器
                        UPDATE public.daily_gallery_user_passwords SET locked_browser_id = v_identifier WHERE id = v_user_pwd.id;
                        v_expires_at := v_user_pwd.expires_at;
                    ELSIF v_user_pwd.locked_browser_id NOT LIKE '%' || v_identifier || '%' THEN
                        -- 已锁定在其他浏览器
